    ...
    function exportMegaModel(core, libraryRoot, callback) {
        ...
        function getNodeData(path, next) {
            ...
            core.loadByPath(root, path, function (err, node) {
                ...
                jsonModel.elements.nodes.push(jnode);
                ...
                for (var ptr in pointers) {
                  jsonModel.elements.edges.push(
                    { source: guid, target: pointers[ptr],
                      type: "points-to", pname: ptr } );
                }
        ...
        function postProcessing() {
            ...
            for (var edge_key in jsonModel.elements.edges) {
                var edge = jsonModel.elements.edges[edge_key];
                switch (edge.type) {
                  case "points-to":
                  ...
                  break;
                  default:
                      console.log("unknown pointer type : " + edge.type);
                }
        ...
        function postProcessPointerTargetEdge(edgeObj) {
          var targetPath = edgeObj.target;
          if (pathCache[targetPath]) {
            edgeObj.target = pathCache[targetPath];
          }
        }
